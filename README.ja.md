...
### 画面共有 / カメラ共有（Codex・Claude Code）

タイトルバーの共有アイコンから、画面やカメラの静止画像を現在のCodexまたはClaude Codeの会話に共有できます。macOSの画面共有では、画面全体・ウィンドウ・範囲選択の各モードを選べます。範囲選択では、共有する領域を移動・リサイズできます。カメラ共有では、使用するカメラを選んで共有を開始します。

更新間隔は**10〜180秒**です。Codexでは画像が会話に追加され、短い間隔ほどトークン消費が増える場合があります。Claude Codeでは、共有開始後に画面やカメラについて質問すると、必要に応じて最新画像を取得します。「共有準備完了」は画像を参照できる状態で、Claudeが画像を読んだことを意味しません。**「共有を停止」**で撮影と保存画像の取得を止めます。詳細は[Claude Codeの画面共有](docs/decisions/claude-screen-sharing.md)を参照してください。

**エージェントの指し示しは実験的機能です。** 画面全体の共有中にAIに頼むと、矢印や囲みで場所を示してくれます。Codexで指し示しを使う場合は、Astra以上のモデルを推奨します。ウィンドウ共有・範囲選択・カメラ共有では利用できません。

<p align="center">
  <img src="docs/assets/screen-sharing-controls.png" alt="画面全体・ウィンドウ・範囲選択のタブ、更新間隔、プレビューを設定する画面共有パネル" width="480" />
</p>
...
- 画面共有 / カメラ共有: 静止画像を10〜180秒間隔でCodex・Claude Codeに共有（Claudeは必要時に最新画像を取得）。macOSでは画面全体・ウィンドウ・範囲選択に対応し、画面全体では実験的なエージェントの指し示しも利用可能
