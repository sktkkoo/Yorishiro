...
Yorishiroが作るのは、AIの能力のためのハーネスではありません。AIがそばに**実在する**と感じられるための構造——**Presence Harness（実在感のハーネス）**です。Yorishiroでは、住人は自分の住む環境をリアルタイムに作り替えられます。照明を変え、シーンを切り替え、UIを組み替える。ユーザーも同じ環境を直接操作でき、住人とユーザーがひとつの環境を共有します。

AIと働く時間は、これからもっと長くなります。これは人間とAIの関係がより密接になることを意味すると同時に、そのインタラクションの在り方を問い直すべき時期がきているとも思います。
