...
住人は自分の住む環境をリアルタイムに作り替えられます。照明を変え、シーンを切り替え、UIを組み替える。ユーザーも同じ環境を直接操作でき、住人とユーザーがひとつの環境を共有します。Yorishiroは、AIの能力のためのものではありません。AIがそばに**実在する**と感じられるための構造——Presence Harness（実在感のハーネス）です。
