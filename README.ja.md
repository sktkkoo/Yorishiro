...
- Charminal が API キーを要求することはありません。代わりに、ユーザー環境の Claude Code / Codex の token をそのまま消費します
- デフォルトは Claude Code です。Codex を使う場合は設定画面から切り替えて Reload（Ctrl+R）してください
...
| **ui** | カメラ・ライティング操作パネルなどの UI |
| **ambient-ui** | 常時表示のオーバーレイ（注視表示など） |
| **utility** | 表示を持たない裏方ロジック |

Bundled pack がデフォルトで動作します。ユーザーは `~/.charminal/packs/` に自作 pack を置くことで、住人の性格・空間・反応のすべてを作り替えられます。Pack は hot reload に対応しており、ファイルを保存するだけでアプリを再起動せずに即座に反映されます。ただし `init.js` などの初期設定ファイルは起動時に一度だけ読まれるため、変更後は Ctrl+R で明示的に Reload してください。hot reload がうまく効かない場合も同様に Ctrl+R で確実に反映できます。
...
住人（ターミナル内の Claude Code）は MCP 経由で Charminal 自身を操作できます——表情を変え、scene を切り替え、エフェクトを走らせ、UI を操作する。これは機能ではなく、住人が自分の住む場所を自分で触れるという設計です。
