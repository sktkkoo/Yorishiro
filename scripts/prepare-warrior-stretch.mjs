#!/usr/bin/env node
/** Derive a finite Warrior stretch candidate from unchanged original 30 Hz keys.
 * node scripts/prepare-warrior-stretch.mjs
...
const input = path.join(root, ".motion-review/mixamo-import-20260915/Warrior Idle.vrma");
const output = path.join(root, ".motion-review/mixamo-import-20260915/Warrior Stretch.vrma");
const expected = "7ce88e4cbb5591da1964d45a182180522150b7b277a5292b8d6390cd75ed4d42";
...
const first = 150; // Original FBX time 5.0 seconds, before the arms rise.
const last = 300; // Original FBX time 10.0 seconds, after the arms return.
const sourceTimes = read(animation.samplers[0].input).values;
if (Math.abs(sourceTimes[first] - 5.0) > 1e-6 || Math.abs(sourceTimes[last] - 10.0) > 1e-6)
...
animation.name = "Mixamo finite Warrior stretch candidate";
json.buffers = [{ byteLength }];
json.extras.derivedUnit = {
  sourcePreparedSha256: expected,
  startSec: origin,
  endSec: sourceTimes[last],
  loop: false,
  sourceFirstKey: first,
  sourceLastKeyInclusive: last,
  status: "Candidate; actual-target composition and automatic admission remain separate reviews",
  scope:
    "Exact finite slice. All source body/finger rotation keys and hips XYZ values are retained; only timestamps are rebased. No loop closure, smoothing, masking, gain change, or target contact adaptation.",
};
...
const report = {
  schemaVersion: 1,
  command: "node scripts/prepare-warrior-stretch.mjs",
  source: {
...
await fs.writeFile(
  path.join(root, "docs/decisions/warrior-stretch-derivation.json"),
  `${JSON.stringify(report, null, 2)}\n`,
);
await fs.writeFile(`${output.slice(0, -5)}.report.json`, `${JSON.stringify(report, null, 2)}\n`);
