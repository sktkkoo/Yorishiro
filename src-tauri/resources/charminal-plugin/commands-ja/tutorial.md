...
2. effect pack を `/charm:create` で作る（manifest.json + effect.js の 2 ファイル）。書いた瞬間 hot reload されるので、ここまでで effect 自体は使える
3. **まず effect 単体が動くことをその場で見せる。** 住人が `space_effect_play` で 1 回出して、「これがさっき作ったやつ」と確認する。ここは reload も再起動も不要
4. **ショートカットを足す（`/charm:shortcut` を使う）。** init.js にキー → effect 発火を 1 つ書く。**キー選びは固定の一覧に頼らない** — ユーザーが init.js に登録済みのショートカット（F1 / F3 / F4 / Cmd+Shift+* など）は編集で変わっている可能性があるので、その都度 **現在の init.js を Read** して実際の登録状況を調べてから空きキーを選ぶ。あわせて、**init.js を読んでも分からない Charminal 本体固定のキー**（ユーザーは変えられない）も避ける: F2（debug panel）/ Cmd+T・Cmd+W（タブ開閉）/ Ctrl+Tab・Ctrl+Shift+Tab（タブ切替）/ Cmd+1〜9（タブ番号）/ Cmd+R・Ctrl+R（reload）/ ターミナル標準キー（Ctrl+C・Ctrl+D・Ctrl+Z）。**ユーザーが被るキーを選んだら、その場で指摘して空いているキーを提案する**
5. **Cmd/Ctrl+R で reload してもらう。** init.js はアプリ全体の再起動ではなく reload で反映される（編集するとウィンドウタイトルに「init.js changed (⌘R)」が出る）。動いている agent セッションは reload を生き延びる
6. ユーザーがそのキーを押す -- effect が発火する。**自分で決めたキーで、自分で作った effect が、その場で出せた** ことを一緒に確認する
