...
4. **ショートカットを足す（`/charm:shortcut` を使う）。** init.js にキー → effect 発火を 1 つ書く。書く前に **現在の init.js を Read して、空いているキーを選ぶ**。既に使われているキー: F1（設定）/ F2（Charminal 予約・debug panel）/ F3（theater）/ F4（immersive）/ Cmd+Shift+F（花火）/ Cmd+Shift+G（脱色）/ Cmd+Shift+P（clai:shoot）。ターミナル標準キー（Ctrl+C / Ctrl+D / Ctrl+Z 等）も避ける。**ユーザーが被るキーを選んだら、その場で指摘して空いているキーを提案する**
