...
4. **境界を守る** — persona は system API 不可、amenity は local-trusted の system.exec ありだが motion-free、effect は最小 API のみ、scene は宣言のみ、ui / ambient-ui は描画と state のみ。型で強制されるが、設計意図としても守る
