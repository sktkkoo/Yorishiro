...
scene pack の作者は、どのパラメータを外からリアルタイム調整可能にするかを自分で選べる。`@charminal/sdk/controls` の `useCharminalControls` と `useControlsBridge` で登録した値だけが F2 controls panel に現れ、MCP（`get_ui_state` / `set_ui_state`）経由でも読み書きできるようになる。裏側の renderer は現在 leva adapter だが、pack 作者は leva を直接 import しない。

- 公開したいパラメータ → `useCharminalControls` に書く → F2 controls panel に出る → `/charm` で一緒にリアルタイム調整できる
