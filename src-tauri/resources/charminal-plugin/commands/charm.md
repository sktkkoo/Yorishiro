...
既存の `allow` 配列に 4 行を追記するだけです（他の設定は変えない）。`init.js` は keyboard shortcut などを仕掛ける startup script（後述）。
...
※ pack 以外に **`~/.charminal/init.js`** という層もある（Emacs の `init.el` 相当）。keyboard shortcut / startup hook / inline での pack 登録用途。詳細は後述「init.js」。
...
- `get_ui_state({packId?, key?})` — UI pack の state を読む。packId 省略で active UI、key 省略で全 snapshot
- `set_ui_state({packId?, key, value})` — UI pack の state を書き、対象 UI pack に通知
...
## init.js（keyboard shortcut / startup hook）
...
- **起動時に 1 度だけ effect を走らせる**（startup animation）
- pack の境界に収まらない自由記述が必要（pack は validator を通すので、形式的な registration を跨ぐ自由度が要るとき）

通常の persona / harness / effect / scene は pack として書く。init.js に載せるのは「pack では書けないもの」だけに留める。

### Context API

default export 関数は `CharminalInitContext` を受け取る。持っているのは 3 つだけ：
...
手続きで `window.addEventListener` / `setTimeout` / `fetch` など、生 JS の API は全部使える。

### 例：Cmd+Shift+F で fireworks
...
`capture: true` で xterm.js が key を消費する前に拾える。`preventDefault + stopImmediatePropagation` でターミナルに届かないようにする。

built-in effect の kind は `shake` / `flash` / `particles` / `fireworks` / `text-physics` / `text-glitch`（詳細は SDK README「Built-in effects」）。user effect pack も同じ API で id を `kind` に渡せば叩ける。

### 境界

- init.js が throw しても Charminal 本体は落ちず、dev-log に残して続行する
- init.js の **hot reload は今のところ無い**（Phase 1-b 時点）— keydown listener のような side effect を hot swap するには leak 防止の設計が要るため、Phase 1-c で正面から扱う。変更したら Charminal を再起動する
- context は最小限（register 2 つ + dispatch 1 つ）のみ。`system` / `character` / `voice` / `space` の高位 API は持たない。それらが要るなら harness pack に移す
