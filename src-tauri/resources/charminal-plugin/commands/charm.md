...
user persona pack は `~/.charminal/packs/<id>/` に **manifest.json + persona.md + persona.js の 3 ファイル**を置く。persona は **multi-active**（複数同時に load される）。scene とは異なり `activePersona` 設定はなく、load された persona はすべて同時に活性化する。
...
1. user と相談して persona の id / name / 性格の方向性を決める。
   **persona は multi-active 前提**（memory: `feedback_persona_multi_active_confirm`）：
   既に active な persona がある状態で作成する時は、「差し替え」か「追加」かを必ず user に確認する。
2. bundled template を read：
   `bundled-packs/personas/charminal-default/persona.md`
3. user pack directory に 3 ファイルを用意：
...
4. hot reload で Charminal runtime が自動 pickup。`list_packs()` MCP tool で status 確認可能。
