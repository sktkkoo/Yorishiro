...
Common camera の `camera.x` / `camera.y` / `camera.z` を `controls_set({ scope: "common", path, value })` で書くと、tracking は自動で Off になり実カメラへ即反映される。滑らかにカメラを動かすデモは `scene_camera_set(...)` を使う。
