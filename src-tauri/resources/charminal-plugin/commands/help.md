...
| **harness** | 環境への自動作用（通知、clipboard 等） | multi | — |
...
- harness / effect / ambient-ui は **multi-active**：loaded されればそのまま動く（harness）、persona handler から呼ばれる（effect）、常時表示される（ambient-ui）
...
| `<kind>.js` | pack 本体（persona.js / harness.js / effect.js / scene.js / ui.js / ambient-ui.js） |
...
{
  "id": "<pack-id>",
  "type": "<persona | harness | effect | scene | ui | ambient-ui>",
  "version": "0.1.0",
...
| ファイル | 内容 |
|---|---|
| `src/sdk/context.d.ts` | PersonaContext / HarnessContext / EffectContext / UiContext / AmbientUiContext |
| `src/sdk/reaction.d.ts` | DispatchEvent / TriggerMatch / ReactionType |
| `src/sdk/pack.d.ts` | PersonaDefinition / HarnessDefinition / EffectDefinition / ScenePackDefinition / UiPackDefinition / AmbientUiPackDefinition |
...
| pack 種類 | 使えない API | 理由 |
|---|---|---|
| **persona** | `ctx.system.*` | 環境操作は harness の責務 |
| **harness** | `ctx.character.*` / `ctx.voice.*` / `ctx.space.*` | 表現は persona の責務（motion-free） |
...
| **ambient-ui** | persona / harness / system API | renderer と attention 情報のみ |

- harness からキャラを動かしたい場合 → **Twin-trigger co-emission**（harness の custom trigger が persona 側の reaction も emit、persona handler が拾う）
