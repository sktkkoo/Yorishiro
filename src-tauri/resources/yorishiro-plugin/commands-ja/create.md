...
1. 切替先の姿を確認する。切替先 pack に `~/.yorishiro/packs/<id>/avatar.vrm` があれば**自動で適用される**（何もしなくてよい。user には「〈name〉の姿に切り替わる」と一言添えれば足りる）。同梱の姿と**別の姿にしたい場合だけ** user にパスを貼ってもらい、`vrm_validate({ "path": "<パス>" })` で**この時点で**検証する（無効なら理由を伝えて、再入力か「姿はそのまま」を選んでもらう）。モデルの切替はここでは行わない——検証で返ったパスを手順 5 の vrmPath で渡すと暗転中に切り替わり、カーテンが明けたときには新しい姿になっている
