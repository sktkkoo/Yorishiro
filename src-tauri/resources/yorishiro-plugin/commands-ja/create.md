...
- `/yori:create` が作る `.js` / `.tsx` pack は **local trusted の `trusted-main-thread-js`**。Charminal public registry 用 artifact ではなく、sandbox 済み・review 済み・公開配布用 pack として扱わない
...
最後に、作成した pack は local trusted code であることを user に伝える。共有したい場合は source code として公開できるが、Charminal の public registry や `/yori:prepare-publish` はまだ未提供である。
...
どの scene pack を active にするかは `scene_activate` で user が明示的に picks する（pack 側の自己申告はしない、Design B — config picks）。`scene_activate` は current project root が解決できる場合は `~/.charminal/config.json` の `sceneByProject` に、解決できない場合は global fallback の `activeScene` に永続化する。例：
...
> packaged build では source tree（`src/`・`bundled-packs/`・`docs/`）はディスク上に無い。bundled pack のソースは `bundled_example_read` MCP tool（id は `list_packs`）で読む。`~/.charminal/sdk.d.ts`（型）と `~/.charminal/sdk-guide.md`（ガイド）は常に読める。以下の `bundled-packs/` / `docs/` path は cwd が Charminal repo のときに有効。

- `~/.charminal/sdk.d.ts` — SDK 型定義一式（pack definition / context。毎起動で再生成）
- `~/.charminal/sdk-guide.md` — pack 作者向けガイド（idiom / twin-trigger co-emission。毎起動で再生成）
