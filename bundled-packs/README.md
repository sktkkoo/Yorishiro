...
## ambient-ui/（Phase 1c 追加予定）

Ambient UI pack（6 つ目の pack kind）。primary UI を占有せず、複数 pack が重なる **multi-active** overlay 層。`ambient-ui-pack-registry` が enable / disable / getActiveSet を管理する。Phase 1a で SDK と registry は実装済み（`src/runtime/ambient-ui-pack-registry/`）。実物の bundled pack は **Phase 1c** で `bundled-packs/ambient-ui/attention-aura/` に追加される。

- **attention-aura**（予定）— `AttentionSnapshot` を受け取り、注目源の方向・強度を aura として overlay 描画する。attention-runtime が publish する snapshot を subscribe し、canvas overlay に反映。
