...
### 自分の pack を作るときのパラメータ公開

ユーザーが scene pack を作るとき、どのパラメータを leva パネルに公開するかは自分で選べる。`useControls` で登録した値だけが leva に現れ、MCP（`get_ui_state` / `set_ui_state`）経由でも読み書きできるようになる。

つまり：
- 公開したいパラメータだけを `useControls` に書く → leva パネルに出る → /charm で一緒に調整できる
- 公開しないパラメータはコード内のローカル変数のまま → 固定値として動く

「どのパラメータを外から触れるようにする？」と聞いてもらえれば、一緒に設計できる。調整したくなったら後から公開パラメータを増やすこともできる。
