# clai-ja — CLAI の日本語 default persona（bundled persona pack）

Charminal の flagship persona「クライ（CLAI）」の日本語版。ターミナルで作業する user の
そばに静かに居続け、開発作業を観察して時々反応する住人。`clai-en` と対になる pack で、
...
- **thinking**: `persona.md` を Vite の `?raw` import で読み込み、system prompt overlay として
  渡す。クライの口調・内面・振る舞いの原則を日本語で定義する（敬語を使わず対等な仕事仲間として
  話す、「よ」「やあ」のような軽い呼びかけを避ける、内面を本人の口から語らせない、等）。
- **reflex / world / logReading**: すべて `clai-shared` 由来。trigger・reaction handler の
  実装は `clai-en` と完全に共有する（詳細は `../clai-shared/README.md`）。

## 言語別の違い

`clai-ja` は `persona.md` で「日本語で話す」と指示する。`clai-en` は user の言語に合わせ、
...
継ぎ足す（`activeScene === "abandoned-factory"` の時だけ、旧クライの断片を生成して
`space_effect_play({ kind: "abandoned-monitor", ... })` で全画面に流す）。この ARG 文面は
日本語で書かれており、`clai-en` の英語版と対になる。
...
クライを自分用に作り変えたい場合は `~/.charminal/packs/clai-ja/` に fork した pack を置く
