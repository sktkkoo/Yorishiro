...
- `~/.charminal/config.json` の `activeUi` を `"immersive"` にする（user が explicit に picks する永続選択）
...
`~/.charminal/packs/immersive/` 配下に同 id の pack を置けば、bundled を override する形で改変可能。`feedback_pack_override_pattern` 参照。
