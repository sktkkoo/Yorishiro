...
- **キャラクター**: VRM body / Persona / Scene。VRM は独立した chooser で検索・サムネイル・作者・利用条件を確認し、固定 footer の明示的な「切り替える」でだけ変更する。import は候補追加までで即時適用しない。選択中でない imported VRM は確認後に一覧と AppData から削除でき、存在しない active path は通知後に自動整理する
