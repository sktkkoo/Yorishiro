...
外力 secondary response の純粋な物理計算と collision helper は workspace package `packages/yoromeki/` に分離し、`src/core/body/stagger-adapter.ts` だけが Three.js / VRM 座標へ接続する。
...
| `runtime/presence-target/` | presence-target pure resolver。active pack 宣言 / host 既定から presence surface を typed result（`ApplyPresenceResult`）で解決し、未宣言時に loud-unavailable を返す（shell named-surfaces P2） |
...
(foundational)  (foundational)
 ...
 core/body/stagger-adapter ──▶ packages/yoromeki
       (VRM/Three adapter)       (plain data only)
```
