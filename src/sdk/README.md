...
pack 作者は `leva` を直接 import せず、SDK controls を使う。現在の F2 controls panel は
裏側で leva adapter を使うが、pack API と UI renderer は分離されている。

```typescript
import { controlFolder, useCharminalControls } from '@charminal/sdk/controls';

const [controls, setControls] = useCharminalControls('lighting', () => ({
...
`useControlsBridge(packId, controls, setControls)` と組み合わせると、F2 controls panel と
