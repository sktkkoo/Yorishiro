...
| `surface-registry/` | host 所有の named surface → DOM HTMLElement 対応表（"shell" / "character"）。layout / MCP / presence の DOM 解決を querySelector 直依存から名前ベースへ（shell named-surfaces P1） | `surface-registry.ts` | |
