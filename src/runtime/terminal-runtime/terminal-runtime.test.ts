...
describe("TerminalRuntime", () => {
  ...
  it("attached session の live respawn 後にも PTY resize と theme refresh を送り再描画を促す", async () => {
    const runtime = getTerminalRuntime("shell-1");
    ...
    runtime.updatePtyParams({ spec: shellSpec, cwd: "/work/new" });
    await flushMicrotasks();

    expect(mockState.sessionSpawn).toHaveBeenCalledWith({
      sessionId: "shell-1",
      spec: shellSpec,
      cols: 80,
      rows: 24,
      cwd: "/work/new",
      onOutput: mockState.channels[0],
    });
    expect(mockState.sessionResize).toHaveBeenCalledWith({
      sessionId: "shell-1",
      cols: 80,
