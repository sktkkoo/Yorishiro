...
- 選択が未保存で、対応 agent が1つだけ見つかった場合は自動選択します。複数ある場合は使うものを選びます。保存済みの選択は、その実行ファイルが見つからなくても自動変更しません。
- 未導入の agent の「インストール」を押すと、公式インストーラーを取得・実行します。Codex と Claude Code のボタンは独立しており、両方の導入も可能です。それぞれのボタンを押すまで導入は始まりません。
...
- 外部で導入する場合は「公式セットアップ」を開くか、表示されたコマンドをコピーしてご自身で実行し、戻って「再確認」を押します。失敗時は「インストールの詳細」を開いて確認し、再試行するか公式の手順を利用してください。
