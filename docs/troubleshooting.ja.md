...
| `~/.charminal/runtime-plugin/` | Claude Code / OpenCode 起動時に渡す生成済み Charminal command plugin |
| `~/.codex/plugins/cache/charminal-local/` | `$charm-*` skills 用の Charminal local Codex plugin cache |
...
Charminal アプリ本体を削除しても、ユーザーデータや生成済み agent 統合 cache は
自動削除されません。通常のアンインストールでは設定や拡張データを残し、再インストール
で再利用できるようにする一般的な desktop app の挙動に合わせています。

Charminal commands は、Charminal が選択中 agent を起動するときだけ注入されます。
Charminal が起動していない場合、Claude/OpenCode の command plugin は渡されず、
Codex plugin cache も Charminal が起動時に渡す `-c` 有効化 flag が無いため有効化されません。
...
場合は、まず修正ボタンか Charminal update command で AI 修復を試してください。すべての
