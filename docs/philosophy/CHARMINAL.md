...
転機は、ターミナルのテキストが物理シミュレーションで崩れ落ちた瞬間です。キャラクターが銃を撃つモーションに合わせて、画面の文字が重力で落下する。技術的には単純ですが、動いたとき「UI を壊してもいいんだ」と気づきました。
...
意識層と反射層は独立して動きます。でも無関係ではありません。反射層で起きたこと（顔を顰めた、視線がさまよった、姿勢を変えた）はログに記録され、Claude Code は必要なときにそれを読めます。
...
コーディングエージェントは答えてくれますが、それは応答であって、共に居ることとは違います。
...
ここに書いたことの多くは、作りながら気づいたことです。このドキュメントは現在地の記録です。
