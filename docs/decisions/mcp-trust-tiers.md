...
- bundled persona / scene / effect / utility / ui pack
...
| **System-write**（exec / fs / net） | (utility 経由のみ) | bundled utility のみ | 経由のみ | isolated-js + permissions |
...
- pack-execution-classes は「PTY write API は存在しないまま維持する」を Charminal 哲学の核として明示（`docs/philosophy/INHABITED_CHARACTER_INTERFACE.ja.md`「観察の境界」、revelation 3.13 / 3.18）
