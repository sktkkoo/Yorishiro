...
`docs/philosophy/PHILOSOPHY.ja.md` 「触れるものと、触れないもの」は、固い核（Rust IO 層 + TS runtime / SDK / core primitive）と生きた表層（user pack、`/charm` 経由の AI 編集）を分けた。
...
`docs/philosophy/PHILOSOPHY.ja.md`「壊さないこと」は、user の積み重ねた pack / 設定 / 関係を破壊しない態度を philosophy として引き受けた。
...
| **Sensitive-read**（user 由来の生データ） | terminal reference の解決（user が Cmd+click / Option+Shift+drag で指した端末テキスト） | ✅ | ✅（user gesture gated） | 明示 grant（default-open にしない） |
...
- mapping 生成は `TerminalRuntime.addTerminalReference`（**private**）のみ。呼び出しは Cmd+click / Option+Shift+drag の host DOM handler だけで、public / SDK / MCP に追加経路は無い → pack/AI は mapping を作れない
...
source: `src/runtime/terminal-runtime/terminal-runtime.ts:782`（private addTerminalReference）, `:692` / `:813`（capture path）, `src/runtime/charminal-mcp/tool-handlers.ts:256`（read-only resolve）
