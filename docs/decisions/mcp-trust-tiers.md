...
`docs/philosophy/CHARMINAL.ja.md` 「触れるものと、触れないもの」は、固い核（Rust IO 層 + TS runtime / SDK / core primitive）と生きた表層（user pack、`/charm` 経由の AI 編集）を分けた。
...
`docs/philosophy/CHARMINAL.ja.md`「壊さないこと」は、user の積み重ねた pack / 設定 / 関係を破壊しない態度を philosophy として引き受けた。
