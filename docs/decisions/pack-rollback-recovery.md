...
> - **P3**：~~content-addressed store~~（git2 移行で解決済）、manual `mark_good`（pin→prune 保護＋restore 候補強調）、diff preview。
...
- 2026-06-06 rev.9: storage を full-copy から git2（libgit2 vendored）に移行。content-addressed store（P3 目標）が解決。cohabitation を config.json から `cohabitation.json` に分離（baseline skip の正確化）。config.json は git 追跡するが watcher trigger 対象外（設定変更のノイズ防止）。詳細は `git2-embedded-snapshot-store.md`。
