...
Yorishiro は動作中に書き換えられる生きた系。ただし系の全てを生きた系にはしない。**核は固く、表層だけが生きている**。住人を停止させずに住環境を変えられることが要件。
...
- **生きた表層**: `~/.yorishiro/packs/` 以下の pack layer（persona、effect、voice、body、scene）。user が書けば live に反映、AI も `/yori` 経由で書き換えに加わる
- **user が触れるもの**: pack layer、config、`/yori` 経由の AI との対話。これ以外は触らない（禁じているのではなく、触っても住人のためにならない）
...
## Yorishiro が Emacs と違う点

1. **書き換える主体に AI が加わっている**。user が `/yori` で AI に指示 → AI が pack ファイルを書き → file watcher が拾って register。user と AI が一緒に住人を書いている状態
2. **住人自身が自身の住む系を観察し操作できる**（自己言及的構造）。住人の身体と環境を同じ MCP tool 群で公開し、住人が自分の手で読み・変える。user も同じ tool に手を伸ばす。詳細は `docs/philosophy/PHILOSOPHY.ja.md`
...
- `docs/philosophy/PHILOSOPHY.ja.md` — 自己言及的構造の詳細
