...
Yorishiro は Body built-in の idle procedural motion（呼吸 / sway / head drift / posture）を `motionIntensity` でスケールする。

- `~/.yorishiro/config.json` の field は `motionIntensity: number`
...
- `src/runtime/yorishiro-mcp/tool-handlers.ts` — `motion_intensity_set` の TS handler
