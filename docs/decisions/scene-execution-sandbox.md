...
MVP では Emacs と同じ local trusted model を採る。GitHub 等で共有された pack は public-registry artifact ではなく、user が `~/.charminal/packs/` に置いて実行する **local trusted code** である。
...
現時点の Charminal には public pack registry、in-app community pack install、`/charm:prepare-publish` が無い。したがって MVP で GitHub 等を通して共有される pack は、審査済み community artifact ではなく **local trusted pack の非公式共有** として扱う。

導入した user は、pack source を読んだ上で自分の `~/.charminal/packs/<pack-id>/` に置く。これは Emacs の elisp package / `init.el` に近い trust model であり、sandbox 安全性ではなく user の明示的な local trust に依存する。
...
MVP では、この品質が必要な scene pack は `trusted-main-thread-js` として扱う。source が `local` なら user の自己責任、`bundled` / `curated` なら Charminal 側の責任で review する。`community` source の `trusted-main-thread-js` を安全表示して配布することはしない。
...
- 身体の最終描画を scene pack 側 Worker に移すため、Charminal の body runtime と scene runtime の所有境界が割れる。
...
Charminal の current user pack も同型である。`~/.charminal/packs/` に置かれた JS は local trusted code として WebView runtime に参加する。まだ public registry / checker / review chain / sandboxed install UX が無い段階で、これを community-safe と呼ばない方が責任境界が明確になる。
...
「GitHub で source を共有できる」と「Charminal が安全な community distribution として扱う」は違う。後者には `pack-execution-classes.md` の declarative / isolated-js / trusted-main-thread-js、source classification、registry integrity model、permission diff、checker が必要である。
...
- **visual impersonation / なりすまし対策を security control として戻す** — Charminal は単一ユーザー・ローカルアプリで、破壊操作は host approval UI で gate する。scene の描画分離は security ではなく layout / performance / distribution の問題として扱う。

## この決定の implication / 制約

- README / pack 作者向け docs では、GitHub 等で共有された pack を「local trusted code」「自己責任」と明記する。
- `~/.charminal/packs/<pack-id>/` へ手動配置する導入方法は書いてよい。ただし public registry / in-app community install があるように書かない。
...
- 2026-05-19: 初版。scene / shader pack の sandbox 議論を MVP 方針として整理。Emacs 的 local trusted sharing を採り、GitHub 等の pack 共有は `~/.charminal/packs/` への自己責任導入として document する。`isolated-js` scene は future public distribution 用の自己完結 layer、Worker 側 VRM rendering は MVP 外とした。
