...
user amenity pack の `ctx.system.exec()` は `sh -c` 経由で任意の shell command を実行できる。これは local user pack（`trusted-main-thread-js`）のみに許可される。community pack からは TS 層で明示的に block する。呼び出しは pack id 付きで audit log に記録する。ただし高頻度 polling の成功ログだけは `quiet` で抑制できる。
...
`ExecOptions.quiet` は、`music-shelf` の now-playing polling のような高頻度・期待成功の読み取り処理で stdout/stderr を埋めないための例外。`quiet` でも error / timeout / non-zero exit は記録する。
...
### 5. Yorishiro の PATH を継承

exec は Yorishiro プロセスの PATH（`build_path_env()` で構築、Homebrew 等のディレクトリを含む）を継承する。agent adapter の extra path dirs も含まれる。これにより `osascript`、`git`、`brew` 等のコマンドが user pack から使える。
...
## Bundled amenity の default-on / disable 方針

bundled amenity は原則 default-on。ユーザーが `disable_pack` した場合は `disabledPacks` を尊重し、次回起動時に active に戻さない。

| pack | 登録時 enable | 理由 |
|---|---|---|
| pomodoro | **する** | system 権限なし。初回体験の核。pomodoro-ui が依存 |
| music-shelf | **する** | bundled code。状態 polling は tool 使用後のみ開始し、読み取り系 exec は `quiet` で成功ログを抑制 |

無効化の手段：
- MCP `disable_pack("music-shelf")` を呼ぶ
- 設定画面の on/off トグル（別 scope で実装予定）

`disabledPacks` は明示 disable の永続化だけに使う。default-on の bundled pack を ghost user pack として表示しないよう、list/diagnose 側では bundled entries と重複する disabled id を user disabled entry にしない。
...
- 2026-06-04: music-shelf を default-on に変更。high-frequency polling の成功ログを `ExecOptions.quiet` で抑制する例外を追加。
