...
`remove_vrm_avatar` も raw path ではなく同じ catalog basename ID だけを受け、`$APPDATA/avatars` 直下の管理用コピーだけを `remove_file` する。directory は削除せず、symlink は追跡せず link 自体だけを削除する。UI は bundled Yori と使用中 avatar を削除対象にせず、「一覧から外れるが、読み込み元の VRM file は削除されない」と明示して確認する。保存済み active path が存在しない場合は、「見つからないため一覧から削除した」と通知し、stale localStorage 参照を Yori へ自動復帰する。実在するが破損・読取不能な file は自動削除せず invalid row として残し、利用者に判断させる。
