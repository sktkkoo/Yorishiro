...
- **GLB spec 違反のファイルは弾かれる**。宣言長 != 実サイズ（trailing padding 等）の VRM は reject。主要 exporter（VRoid / UniVRM / Blender VRM）は spec 準拠で通る。bundled `CLAI.vrm`（22.7 MB）は宣言長 == 実サイズで通ることを確認済み
