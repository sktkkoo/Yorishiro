...
- **`command_syntax() -> CommandSyntax`**：charm コマンドの記法 `<prefix>charm<separator><name>`（Claude `/charm:create` / Codex `$charm-create` / OpenCode `/charm-create`）。`opencode.rs` の template rewrite はこの宣言から導出し、`AgentDescriptor.commandSyntax` として `list_supported_agents()` で公開する。
