...
1 つの `RendererAPI` method に押し込むと、どれか 1 つに偏って他が描けない。memory: `feedback_separate_conceptually_distinct_systems` — 表面的類似で統合しない。
...
- philosophy: `docs/philosophy/PRESENCE_HARNESS.ja.md` 「六要素 > 空間」
...
- memory: `feedback_large_diff_tradeoff` / `feedback_inductive_tuning_params` / `feedback_separate_conceptually_distinct_systems`
