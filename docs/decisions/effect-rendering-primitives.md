...
ただし **init.js は例外**: keyboard shortcut / startup hook のため、生 DOM access が許される自由記述層として別扱い（`/yori` doc で明示）。
...
- source: `src/runtime/user-pack-loader/init-script.ts` — init.js runner（`YorishiroInitContext`）
- doc: `src-tauri/resources/yorishiro-plugin/commands-ja/create.md` / `commands-ja/shortcut.md` — `/yori:create` での effect pack と `/yori:shortcut` での init.js ガイド
