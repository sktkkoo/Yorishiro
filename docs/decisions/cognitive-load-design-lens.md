...
- **domain boundary を型レベルで強制**：`PersonaContext` / `HarnessContext` は意図的に別型 — 型エラーで境界違反を検出（critical-constraints §2 「Harness は motion-free」の根拠）
...
- 「便利そう」だけの abstraction は merge しない（[memory: feedback_large_diff_tradeoff](../../.claude/projects/-Users-user-Charminal/memory/feedback_large_diff_tradeoff.md) と同方向）
...
- `feedback_large_diff_tradeoff.md` — large diff の judgement、本原則の前駆形
- `feedback_separate_conceptually_distinct_systems.md` — 分離が正解だった事例（本原則の逆方向）
