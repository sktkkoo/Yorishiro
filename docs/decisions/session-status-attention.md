...
Yorishiro は起動ごとに loopback の動的 port と instance UUID token を確保する。shell session spawn 時には `YORISHIRO_SESSION_ID` / `YORISHIRO_HOOK_PORT` / `YORISHIRO_HOOK_TOKEN` / `YORISHIRO_HOOK_LAUNCH_ID` / `YORISHIRO_AGENT_SHIM_ROOT` を環境へ入れ、`~/.yorishiro/shell/session-shims/<session>/` を `PATH` 先頭へ prepend する。この directory に `claude` / `codex` shim を生成する。hook server は instance token が一致しない signal と、現在の agent launch ID に一致しない stale signal を拒否するため、production / dev の同時起動や同一 PTY の respawn でも attribution が交差しない。hook server を確保できない場合は app / PTY を止めず、hook 観測機能だけを unavailable として degrade する。
...
- 2026-08-15 rev.12: hook server を固定 port から per-instance dynamic loopback port へ変更し、instance UUID token と per-launch UUID で cross-instance / stale-process signal を拒否する。bind 失敗時は app / PTY を継続し、hooks unavailable へ安全に degrade する。
