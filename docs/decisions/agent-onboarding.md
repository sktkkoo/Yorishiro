...
Codex / Claude Code は個別の明示操作で導入する。画面の説明は短くし、アカウント・料金の共通説明は上部に一度だけ表示する。公式リンクとコマンドは「詳細」にまとめる。
...
- アカウント・料金・規約の共通説明は、利用条件の文の直下に一度だけ表示する。公式ガイド・規約リンクと手動導入は初期状態で閉じた「詳細」に置く。手動導入の見出しは「インストールコマンド」/ `Install command` とし、「コピー」/ `Copy` はコマンドの右側に置く。
...
- 2026-09-20（共通説明）: アカウント・料金・規約の説明を各 agent の詳細から取り出し、利用条件の文の直下へ一つにまとめて常時表示。
