...
**Implementation note**: `pack` の public registry / community 公開配布機能と `/yori:prepare-publish` はまだ準備中であり、現行 release で利用できる機能ではない。この文書の publish flow / registry review / machine checker は、公開配布を解禁する前に満たすべき設計要件として扱う。
...
|---|---|---|
| `bundled` | Yorishiro 本体に同梱され、release build と一緒に review / ship される pack | 許可 |
| `local` | user が `~/.yorishiro/packs/` に直接置く pack。`/yori:create` が作る pack もここ | 許可。ただし公開配布済みとは表示しない |
| `curated` | Yorishiro 側で publisher / hash / review metadata を allowlist した配布 pack | 条件付き許可。sandbox 済みとは表示しない |
...
`scene` / `effect` / `persona` / `amenity` / `ui` は「その pack が Yorishiro で何を意味するか」を表す。JavaScript の実行権限は制限しない。
...
JS module を評価する時点で任意コード実行である。公開配布で data-only と呼ぶには、host が JSON / data を parse し、schema validation し、Yorishiro runtime が描画・登録・反応実行を担当する必要がある。
...
`isolated-js` は、pack JS を Yorishiro main thread / app realm と分離して実行する class である。
...
- `lockdown()` は Yorishiro app realm ではなく isolated runtime 内でだけ実行する
...
`executionClass` は pack JS / data をどの境界で評価するかを表す。`sandbox` 宣言は script 実行型 pack の capability backend を表す別軸であり、`executionClass` と同列の top-level manifest field として扱う。

能力ラダーは `declarative` → `sandbox: "wasm"` → `sandbox: "native"` とする。短縮表記では `sandbox: "wasm"` / `sandbox: "native"` と呼ぶが、manifest の正規形は `sandbox: { "backend": "wasm" }` / `sandbox: { "backend": "native" }` である。

Phase 0 では schema を先行定義し、未知 backend / 未知 field を fail-closed で reject する。backend enforcement が無い限り、妥当な `sandbox` 宣言を持つ pack も未実装として reject する。詳細は [`pack-sandbox-strategy.md`](pack-sandbox-strategy.md)。
...
- `/yori:create` / `/yori:update` は `.js` / `.tsx` pack を local-only `trusted-main-thread-js` として生成・編集し、公開 amenity / `isolated-js` / unsafe asset / PTY write を作らないよう prompt で明示

未実装：

- `declarative` artifact の data loader / schema registry
- `permissions` manifest schema と enforcement
- `/yori:prepare-publish` 相当の local trusted pack → public artifact 変換 flow
...
## `/yori:create` と公開配布の関係

`/yori:create` は user が自分の環境で素早く試すための authoring flow であり、生成物は default で `source: "local"` + `executionClass: "trusted-main-thread-js"` として扱う。これは「user が自分の `~/.yorishiro/packs/` に置いた code を自分で信頼して動かす」経路であり、community 公開配布の安全基準を満たすことを意味しない。

したがって `/yori:create` で作った pack を公開したい場合は、そのまま registry に載せず、別 flow で publish 用 artifact へ変換する。仮称は `/yori:prepare-publish` とする。

ただし `/yori:prepare-publish` と pack 公開 UI / registry submission はまだ未提供である。現時点では local authoring pack を公開可能形式へ自動変換・提出できないため、ここで定義する責務は公開配布機能を実装する際の acceptance criteria とする。
...
`prepare-publish` は machine-check first にする。人間 review は「機械で確定できない判断」に限定し、CLI / `/yori:prepare-publish` / registry CI で同じ checker を再利用する。checker は pure module として実装し、local で通った candidate が registry 側で別判定にならないようにする。

自動 checker の分類：

| category | hard reject | warning / info |
|---|---|---|
| Manifest / policy | schema invalid、`declarative` + JS entry、community `trusted-main-thread-js`、未実装 `isolated-js`、executionClass downgrade / unsafe change | description 欠落、古い `yorishiroVersion` range |
...
これらは registry review の対象に残す。hard reject が 0 でない pack は registry review に送れない。warning は user に説明し、可能なら `/yori:prepare-publish` が修正案を出す。
...
2. `/yori:prepare-publish` で local trusted pack を publish 用 `declarative` artifact に変換できるようにする
3. `scripts/check-pack` 相当の machine checker を CLI / `/yori:prepare-publish` / registry CI で共有する
...
- `/yori:create` 由来の local `trusted-main-thread-js` pack を変換なしで community publish しようとしている
...
Yorishiro は自身の MCP server を実装し、内部の住人（Claude Code 経由の AI）や外部 client にあらゆる機能を tool として公開する計画である（思想は [docs/philosophy/PHILOSOPHY.ja.md](../philosophy/PHILOSOPHY.ja.md)）。

これと pack-execution-classes は **layer が違う**：

- pack-execution-classes は配布される **pack の JS 実行**を扱う。host (Yorishiro) と guest (pack) の境界。
- self-referential MCP は Yorishiro **host が公開する tool 群** を扱う。host runtime の機能 surface。
...
- philosophy: [docs/philosophy/PHILOSOPHY.ja.md](../philosophy/PHILOSOPHY.ja.md)（self-referential MCP 思想）
...
- 2026-05-16: source classification、declarative hostile data checklist、isolated-js 着手 gate、capability RPC validation、registry trust limitation、SES bypass 時の防御モデル、`/yori:create` と publish 用変換 flow、machine checker の関係、公開配布機能が未提供であることを追記。
