...
それでも Ctrl+C などで中断された paginated history を Codex app-server が fork
できない場合は、元の履歴を変更せず新規 thread を起動する。履歴継承より、
protected main session が stopped のまま回復不能にならないことを優先する。
...
- 2026-08-22: active writer fork も persisted history 破損で失敗した場合の fresh start fallback を追加。
