...
4. **重い one-shot work を interaction frame から外す**: terminal restart は folder label の paint 後に送り、TTS の byte 受け渡しと WAV decode は JS の同期変換を通常経路にしない。
...
- input cursor / focused DOM producer は scan 時だけ rAF で追ってよいが、interval 待機中に rAF を常駐させない。rect が変わらなければ `attention.setSourceTarget()` しない。
...
- `setCwd()` と xterm reset / agent respawn を同じ interaction frame に載せない。表示用 cwd は即時更新し、TerminalWorkspace に渡す fallback cwd と main session launch cwd の差し替え、force restart は after-paint に送る。
- live respawn / attach 後は PTY resize と theme refresh を送り、TUI / terminal screen が空表示のまま残らないようにする。
...
- interaction に直結する terminal restart / xterm reset を after-paint に送る。
