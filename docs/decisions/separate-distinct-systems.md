...
- 同じ「pack」でも：
  - **persona / harness**: runtime-active、event 駆動
...
- Charminal の SDK は AI が読んで pack を書く前提 → **概念ごとに type / API が分かれている方が AI の判断が安定**（PersonaContext と HarnessContext を別型にすることで「harness で character を呼ぶ」という mistake が型 error になる）
...
- ✅ PersonaContext / HarnessContext / EffectContext は別型（共通基底クラスを作っていない）
...
- 関連: [`critical-constraints.md`](critical-constraints.md) §2 Harness motion-free（型レベル境界の例）
