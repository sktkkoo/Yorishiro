...
このディレクトリは、Charminal の設計上の **決定** と **制約** を **topic 軸**で引けるようにした index。非公開別 repo `../Charminal-design-record/` (date-indexed) を補完する。外部の読者はこのディレクトリだけで設計意図を追える。

---

## なぜこれが必要か

design-record（非公開別 repo）は **date-prefixed** で、設計の **過程** を時系列で記録する。一方で実装中によくあるのは：
...
1. **revelation や spec で新しい制約が固まった時**（非公開 design-record の決定を topic 軸に index する）
...
## 関連 reference
- internal design-record file (date prefix、非公開)
...
- [**scene-controls-api.md**](scene-controls-api.md) — Scene Pack の lighting / post effect controls は SDK 経由にし、Leva は公開 API ではなく adapter / debug UI に留める
...
- *UI pack（5 つ目の pack kind）は Plan 3 完了まで shape が変わるため、この topic-indexed index には未 promote。内部 design-record `2026-04-21-ui-pack-single-active.md` を参照（非公開）*
...
- [**pack-execution-classes.md**](pack-execution-classes.md) — Pack `type` は product semantics、`executionClass` は security boundary。declarative / isolated-js / trusted-main-thread-js の 3 class を分離。公開 utility は `isolated-js` 完成後の future scope
...
- *（今後の追加候補）* `twin-trigger-co-emission.md` — utility と persona の正規 idiom（revelation 3.17）
...
- 未整理の思考過程（→ `../Charminal-design-record/`、非公開）
