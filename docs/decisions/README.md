...
- [**session-status-attention.md**](session-status-attention.md) — session ごとの観察 read model（lifecycle / activity / unread / exit / attention）と TabIndicator badge。許可待ち（`input`）は OSC 9/99/777 + Claude Notification hook の二系統で観察し、focus / 出力では消さず、確定入力 or stop/prompt hook で解除する
