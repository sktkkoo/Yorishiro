...
- [**motion-intensity.md**](motion-intensity.md) — idle procedural motion の大きさを `motionIntensity` で調整する。range `0.0`–`3.0` / default `1.0` / default 不変。settings・SDK・MCP に対称公開し、VRMA clip / persona reaction / lip-sync / startle は scale しない
