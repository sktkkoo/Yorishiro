...
Charminal の Terminal は shell（zsh / bash / fish / pwsh）と coding agent（Claude Code / Codex / OpenCode）を session として走らせる。複数 session は tab として保持し、表示は active session だけに絞る。
...
Charminal は session が 2 つ以上ある場合、active session の terminal だけを表示する。
