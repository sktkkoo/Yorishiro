...
| `command-block` | Shell integration の OSC 133 / 633 | `command`, `exitCode`, `durationMs`, `sessionId`, `timestamp` | 失敗 / 長時間完了など noteworthy な command run への反応 |
...
| `window` | Yorishiro window state | `change`, `timestamp` | focus / blur / resize への反応 |
| `scene-change` | active scene の切替 | `fromId`, `toId`, `timestamp` | scene 移行時の一時反応 |
| `yori-command` | `/yori` command | `command`, `timestamp` | command 実行への補助反応 |
...
（`"claude"` / `"codex"` 等、pack 由来は `null`）を host が stamp する。Yorishiro は loop を
観察するだけで制御しない。詳細は [`docs/decisions/loop-presence-layer.md`](../decisions/loop-presence-layer.md)。

`command-block` は `kind: "shell"` かつ `integration: true` の session だけで生成される。
replay 復元中は stale reaction 防止のため dispatch しない。すべての command に反応させず、
persona 側では failed / slow-completed のような noteworthy なものに絞る。
