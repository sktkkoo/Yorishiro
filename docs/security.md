...
This is a map of Yorishiro's trust boundaries and attack surface. It does not
...
Yorishiro is a Tauri app: a TypeScript WebView layer over a Rust host layer. An
...
| User packs (`~/.yorishiro/packs/`) | Local trusted code. Not sandboxed, not reviewed, not a public-registry artifact. Installing a shared pack manually = choosing to run local trusted code. | [`decisions/pack-execution-classes.md`](decisions/pack-execution-classes.md), [`decisions/scene-execution-sandbox.md`](decisions/scene-execution-sandbox.md) |
| Bundled packs | Part of the app, immutable from within Yorishiro. | [`decisions/pack-execution-classes.md`](decisions/pack-execution-classes.md) |
| MCP capabilities | *Intended:* the resident agent controls Yorishiro through tiered MCP tools, with destructive/sensitive capabilities gated by tier. **Tier gating is not yet implemented** (see "Current enforcement status" below). | [`decisions/mcp-trust-tiers.md`](decisions/mcp-trust-tiers.md) |
| Pack ↔ host (IPC) | The Yorishiro SDK is the *supported* authoring surface, but it is not a runtime-enforced sandbox: a local pack shares the host realm and can reach Tauri IPC directly. A pack reaching host IPC directly is a vulnerability only once packs are sandboxed (future community / `isolated-js` class); for today's local trusted packs it is not prevented. See "Current enforcement status". | [`../SECURITY.md`](../SECURITY.md), [`decisions/input-prefill-boundary.md`](decisions/input-prefill-boundary.md) |
...
- **Local config (`~/.yorishiro/config.json`)** — user-owned; treated as trusted local input.
- **Local cohabitation state (`~/.yorishiro/cohabitation.json`)** — user-owned runtime state; treated as trusted local input and kept outside rollback snapshots.
...
Yorishiro has no public pack registry, in-app community pack installation, or
`/yori:prepare-publish`. These must not be described as released until the pack
