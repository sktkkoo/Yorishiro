...
|---|---|---|
| Terminal (PTY) | Observation only. No pack/MCP API can write to the PTY; `terminal_prefill` / `write_terminal_input` style behavior is treated as security-sensitive. | [`decisions/critical-constraints.md`](decisions/critical-constraints.md), philosophy `INHABITED_CHARACTER_INTERFACE.md` |
| User packs (`~/.charminal/packs/`) | Local trusted code. Not sandboxed, not reviewed, not a public-registry artifact. Installing a shared pack manually = choosing to run local trusted code. | [`decisions/pack-execution-classes.md`](decisions/pack-execution-classes.md) |
...
| Pack ↔ host (IPC) | The supported authoring surface is the Charminal SDK, not direct Tauri IPC. A non-bundled pack invoking host commands directly, or bypassing the intended capability path, is a vulnerability. | [`../SECURITY.md`](../SECURITY.md) |

The conceptual basis for "a pathway existing is what makes a boundary" is the
philosophy doc [`philosophy/SELF_REFERENTIAL_MCP.md`](philosophy/SELF_REFERENTIAL_MCP.md).
