...
|---|---|---|
| Terminal (PTY) | Observation only. No pack/MCP API can write arbitrary text to the PTY; `terminal_prefill` / `write_terminal_input` style behavior is treated as security-sensitive. Packs reach the input only through host-owned fixed-prompt verbs and the user-gesture-gated reference marker. | [`decisions/critical-constraints.md`](decisions/critical-constraints.md), [`decisions/input-prefill-boundary.md`](decisions/input-prefill-boundary.md), philosophy `INHABITED_CHARACTER_INTERFACE.md` |
